...
- Convención interna configurable con `--level_convention` (`depth`/`height`).
- El pipeline normaliza `nivel_m` a `nivel_norm` para que todo el ajuste (hs/hd/tau/k) use una convención única.
- Si detecta que el input viene con signo opuesto, aplica flip de signo (`nivel_sign_flip_aplicado=1`).
- Si en un ciclo no se cumple la relación esperada entre `hd` y `hs` según convención, marca `warning_convention_mismatch=1` y no calcula `k` para ese ciclo/período.
