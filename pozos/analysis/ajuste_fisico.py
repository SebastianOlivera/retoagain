...
def compute_k(C: float, hs: float, hd: float, convention: str, eps_denom: float = 1e-6) -> tuple[float, bool, bool]:
    if not (np.isfinite(C) and np.isfinite(hs) and np.isfinite(hd)):
    ...
    if mismatch:
        return np.nan, False, True
    if not np.isfinite(den) or abs(den) < eps_denom:
        return np.nan, False, mismatch

    k_val = float(C / den)
    if not np.isfinite(k_val) or k_val <= 0:
        return np.nan, False, True
    return k_val, True, mismatch
...
def fit_period_cycles(
    ...
    for idx, (s, e) in enumerate(on_segments):
        ...
        k_val, ok_k, mismatch = compute_k(
            C=c_seg,
            hs=h_static,
            hd=hd_fit,
            convention=level_convention,
            eps_denom=eps_denom,
        )
        if not ok_fit:
            k_val = np.nan
            ok_k = False
        mismatch_any = mismatch_any or mismatch
        ...
        cycles.append(
            {
                ...
                "k": k_val,
                "ok_k": ok_k,
                "warning_convention_mismatch": bool(mismatch),
                "rmse": rmse,
                "r2": r2,
                "ok_fit": ok_fit,
