...
def _infer_input_level_convention(df: pd.DataFrame, level_col: str = "nivel_m", on_col: str = "pump_on") -> str:
    on_mask = df[on_col].astype(bool)
    if on_mask.sum() == 0 or (~on_mask).sum() == 0:
        return "depth"
    hd = float(np.nanmedian(pd.to_numeric(df.loc[on_mask, level_col], errors="coerce")))
    hs = float(np.nanmedian(pd.to_numeric(df.loc[~on_mask, level_col], errors="coerce")))
    if not (np.isfinite(hd) and np.isfinite(hs)):
        return "depth"
    return "depth" if hd >= hs else "height"


def _normalize_level(
    dfx: pd.DataFrame,
    target_level_convention: str = "depth",
    level_col: str = "nivel_m",
    on_col: str = "pump_on",
) -> tuple[pd.DataFrame, dict[str, Any]]:
    target = target_level_convention.lower().strip()
    if target not in {"depth", "height"}:
        raise ValueError("level_convention debe ser 'depth' o 'height'")

    inferred = _infer_input_level_convention(dfx, level_col=level_col, on_col=on_col)
    sign_flip = int(inferred != target)

    out = dfx.copy()
    out["nivel_norm"] = -pd.to_numeric(out[level_col], errors="coerce") if sign_flip else pd.to_numeric(out[level_col], errors="coerce")

    return out, {
        "level_convention_usada": target,
        "nivel_sign_flip_aplicado": sign_flip,
        "nivel_es_profundidad_detectado": int(inferred == "depth"),
    }


def _prepare_df(df: pd.DataFrame, smooth_window: int, thr_m3s: float, level_convention: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    dfx = df.copy()
    ...
    if dfx.empty:
        return dfx, {
            "level_convention_usada": level_convention,
            "nivel_sign_flip_aplicado": 0,
            "nivel_es_profundidad_detectado": 1,
        }
    ...
    dfx, conv_meta = _normalize_level(dfx, target_level_convention=level_convention, level_col="nivel_m", on_col="pump_on")
    dfx["nivel_use"] = robust_median_smooth(dfx["nivel_norm"], window=max(1, int(smooth_window)))
    return dfx, conv_meta
...
def compute_cycle_metrics(
    ...
    smooth_window: int = 5,
    level_convention: str = "depth",
) -> pd.DataFrame:
    required = {"ts", "nivel_m"}
    if not required.issubset(df.columns):
        raise ValueError(f"Faltan columnas. Requiere {required} + caudal_ls/caudal_m3s")

    dfx, conv_meta = _prepare_df(df, smooth_window=smooth_window, thr_m3s=thr_m3s, level_convention=level_convention)
    if dfx.empty:
        return pd.DataFrame()

    fit = fit_period_cycles(
        dfx,
        thr_m3s=thr_m3s,
        ts_col="ts",
        flow_col="caudal_m3s",
        level_col="nivel_use",
        level_convention=conv_meta["level_convention_usada"],
    )
        ...
        rows.append(
            {
                ...
                "level_convention_usada": conv_meta["level_convention_usada"],
                "nivel_sign_flip_aplicado": int(conv_meta["nivel_sign_flip_aplicado"]),
                "nivel_es_profundidad_detectado": int(conv_meta["nivel_es_profundidad_detectado"]),
                "warning_convention_mismatch": int(cyc.get("warning_convention_mismatch", False)),
...
def compute_period_metrics(
    ...
    for periodo, part in dfx.groupby("periodo", sort=True):
        ...
        row: dict[str, Any] = {
            ...
            "nivel_sign_flip_aplicado": int(conv_meta["nivel_sign_flip_aplicado"]),
            "nivel_es_profundidad_detectado": int(conv_meta["nivel_es_profundidad_detectado"]),
            "warning_convention_mismatch": int(bool(fit.get("warning_convention_mismatch", False))),
    ...
    desired_order = [
        ...
        "rmse_global",
        "r2_global",
        "level_convention_usada",
        "nivel_sign_flip_aplicado",
        "nivel_es_profundidad_detectado",
        "warning_convention_mismatch",
    ...
    for col in periods_df.columns:
        if col.endswith("_n") or col in {"n_on", "nivel_sign_flip_aplicado", "nivel_es_profundidad_detectado", "warning_convention_mismatch"}:
