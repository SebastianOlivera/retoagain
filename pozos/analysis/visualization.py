...
def plot_flow(df: pd.DataFrame, ts_col: str = "ts", flow_col: str = "caudal_ls") -> None:
    plt.figure()
...
def smooth_flow(df: pd.DataFrame, flow_col: str = "caudal_ls", window: int = 25) -> pd.Series:
    return df[flow_col].rolling(window, center=True).mean()


def plot_smoothed_flow(df: pd.DataFrame, ts_col: str = "ts", flow_col: str = "caudal_ls", window: int = 25) -> pd.Series:
